...
caminho_resultados = r"C:\Users\Dell\Desktop\Modelos Redes Neurais\MLP"
...
with open(log_path, "w") as f:
    f.write("Teste de Hiperparâmetros - MLP\n\n")
...
for hidden_size, num_layers, epochs, batch_size, passos in param_grid:
    ...
    # Criar sequências para MLP
    X, y = [], []
    for i in range(passos, len(dados_entrada)):
        seq = dados_entrada[i-passos:i].flatten()  # achatar sequência
        X.append(seq)
    ...
    # Inicializar modelo
    input_size_mlp = X_train.shape[1]
    modelo = MLPModel(input_size=input_size_mlp, hidden_size=hidden_size, num_layers=num_layers, output_size=output_size).to(device)
    criterio = nn.MSELoss()
